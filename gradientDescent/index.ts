const y = (m: number, x: number, c: number) => m * x + c;
...
const adjustParams = (m: number, c: number) => {
  let mDerivative = 0;
  let cDerivative = 0;
...
const performanceR2 = (m: number, c: number) => {
  let sRes = 0,
    sTot = 0;
  const yMean =
    initialData.reduce((acc, curr) => acc + curr.y, 0) / initialData.length;
...
const performanceR2Adjusted = (m: number, c: number) => {
...
const main = () => {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    if (currCost > prevCost) {
      console.log(
        `\nCost increased from ${prevCost.toFixed(3)} to ${currCost.toFixed(
          3
        )} at iteration ${i + 1}`
      );
      break;
